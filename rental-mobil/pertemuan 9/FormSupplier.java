...
public class FormSupplier extends JInternalFrame {
    ...
    public FormSupplier() {
        super("Data Supplier", true, true, true, true);
        setSize(650, 500);
        ...
        form.setBorder(BorderFactory.createCompoundBorder(
            ...
            BorderFactory.createTitledBorder("Input Data Supplier")));

        form.add(new JLabel("Kode Supplier:")); kodeTF   = new JTextField(); form.add(kodeTF);
        form.add(new JLabel("Nama Supplier:")); namaTF   = new JTextField(); form.add(namaTF);
        ...
        String[] kolom = {"Kode", "Nama Supplier", "Alamat", "Telepon"};
        ...
        scroll.setBorder(BorderFactory.createCompoundBorder(
            ...
            BorderFactory.createTitledBorder("Tabel Supplier")));
        ...
        simpanBtn.addActionListener(e -> {
            boolean ok = DatabaseHelper.simpanSupplier(kodeTF.getText().trim(),namaTF.getText().trim(),alamatTF.getText().trim(),telpTF.getText().trim());
        ...
        updateBtn.addActionListener(e -> {
            ...
            boolean ok = DatabaseHelper.updateSupplier(kodeDipilih,namaTF.getText().trim(),alamatTF.getText().trim(),telpTF.getText().trim());
        ...
        hapusBtn.addActionListener(e -> {
            ...
            if (conf == JOptionPane.YES_OPTION) {
                boolean ok = DatabaseHelper.hapusSupplier(kodeDipilih);
    ...
    void refreshTabel() {
        ...
        try (Connection c = DatabaseHelper.connect();
             ResultSet rs = DatabaseHelper.getAllSupplier(c)) {
            while (rs.next())
                tableModel.addRow(new Object[]{rs.getString("kode_supplier"),rs.getString("nama_supplier"),rs.getString("alamat"),rs.getString("telepon")});
